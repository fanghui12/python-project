...
def search_windows(img, windows, clf, scaler, color_space='RGB',
    ...
    for window in windows:
        img1 = img.copy()
        #print(window)
        cv2.rectangle(img1, (window[0][0],window[0][1]), (window[1][0], window[1][1]), (255, 0, 0), 2)
...
for i, name in enumerate(test_imgs[:4]):
    ...
    for xy_window in xy_windows:
        #print(xy_window)
        windows = slide_window(image,#图片
                               x_start_stop=[600, None],#ROI区域宽度(像素)
                               y_start_stop=[350, 660],#ROI区域高度（像素）
                               xy_window=xy_window,#窗口像素个数（w:h）
                               xy_overlap=(0.8, 0.8))#窗口重叠度

        total_windows.extend(windows)
        window_img = draw_boxes(image, total_windows, color=(0, 0, 255), thick=6)
        # plt.imshow(window_img)
        # plt.show()
    hot_windows = search_windows(image, total_windows, mlp, scaler, color_space='YCrCb',
    ...
    heatmap = apply_threshold(heatmap, 2)#应用阈值
...
#       for xy_window in xy_windows:
#
#         windows = slide_window(image, x_start_stop=[100, 300], y_start_stop=[200, 450],
...
#
#       hot_windows = search_windows(image, total_windows, mlp, scaler, color_space='YCrCb',
#                                  spatial_size=(16, 16), hist_bins=hist_bins,q
#                                  hist_range=(0, 256), orient=orient,
#                                  pix_per_cell=pix_per_cell, cell_per_block=cell_per_block,
#                                  hog_channel='ALL', spatial_feat=True,
#                                  hist_feat=True, hog_feat=True)
