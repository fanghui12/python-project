...
with tf.Session() as sess:
    ...
    while (1):
        ...
        outImage = np.reshape(conv2d_1,(784,32))
        dst = np.zeros((28 * 4,#图片宽*图片行个数
                        28 * 8)) #图片高*图片列个数
        rows_th = cols_th = 0
        for i in range(32):#32个权重图
          fileName = "w" + str(i)
          pinjieimage = np.reshape((outImage[:, i]), (28, 28))
        ...
        outImage = np.zeros((196,64))
        outImage = np.reshape(conv2d_2,(196,64))
        dst = np.zeros((14 * 8,#图片宽*图片行个数
                        14 * 8)) #图片高*图片列个数
        rows_th=cols_th=0
        for i in range(64):#64个权重图
          fileName = "w" + str(i)
          #cv2.imshow(fileName,np.reshape((outImage[:,i]),(8,8)))
          pinjieimage = np.reshape((outImage[:, i]), (14, 14))
          shape = pinjieimage.shape  # 三通道的影像需把-1改成1
          cols = shape[1]
          rows = shape[0]
